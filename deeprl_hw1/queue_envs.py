...
class QueueEnv(Env):
    ...
    def _step(self, action):
        ...
        if action == self.SWITCH_TO_1:
            self.s[0] = 1
        if action == self.SWITCH_TO_2:
            self.s[0] = 2
        if action == self.SWITCH_TO_3:
            self.s[0] = 3
        if action == self.SERVICE_QUEUE and self.s[self.s[0]] > 0:
            self.s[self.s[0]] = self.s[self.s[0]] - 1
            reward = reward + 1
        ...
        """
        return tuple(self.s), reward, False, None
    ...
    def query_model(self, state, action):
        ...
        if action == self.SWITCH_TO_3:
            nextstate[0] = 3
        if action == self.SERVICE_QUEUE and nextstate[state[0]] > 0:
            nextstate[state[0]] = nextstate[state[0]] - 1
            reward = reward + 1


        for i in range(len(self.Prob)):
            nextstate[i+1] = nextstate[i+1] + 1
            outcomes.append((self.Prob[i], tuple(nextstate), reward, False))
            nextstate[i+1] = nextstate[i+1] - 1
